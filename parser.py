...
left_val, err = evaluation_tree(left_node)
if err is not None:
    return None, err

right_val, err = evaluation_tree(right_node)
if err is not None:
    return None, err 
...
if op == "+":
    ...
    if right_val == 0
    return non "Modulo by zero error"
    return left_val % right_val, None
elif op == '^':
    return left_val ** right_val, None
...
if ininstance(result, float) and result.is_integer():
    result=int(result)
